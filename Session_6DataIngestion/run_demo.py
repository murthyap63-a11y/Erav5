...
from data_system_engine import (
    ...
    OpusCurriculumEngine, LearningLedger, DataIntegrityAuditor
)
...
def main():
    ...
    auditor = DataIntegrityAuditor(tokenizer=tokenizer)
    ...
    # code_man - code manifest, text_man - text manifest, eval_man - evaluation manifest
    # code_bin - code binary, text_bin - text binary, eval_bin - evaluation binary
    code_bin, code_man = builder.create_shard(shard_id=1, documents=code_documents, is_evaluation=False)
    text_bin, text_man = builder.create_shard(shard_id=2, documents=text_documents, is_evaluation=False)
    eval_bin, eval_man = builder.create_shard(shard_id=3, documents=eval_documents, is_evaluation=True)

    print(f"Asset Created: {code_bin} | Manifest: {code_man}")
    print(f"Asset Created: {text_bin} | Manifest: {text_man}")
    print(f"Asset Created: {eval_bin} | Manifest: {eval_man}")
    
    #verify shard integrity and manifest correctness
    record_log("INFO", "verifying shard integrity and manifest correctness.")
    # Step B: Run pipeline startup verification checks
    is_textvalid = auditor.verify_shard(text_bin, text_man)
    is_evalvalid = auditor.verify_shard(eval_bin, eval_man)
    is_codevalid = auditor.verify_shard(code_bin, code_man)
    ...
    try:
        ...
        os.chmod(text_bin, 0o644)
        os.chmod(code_bin, 0o644)
        os.chmod(eval_bin, 0o644)
        with open(text_bin, "ab") as f:
            f.write((99).to_bytes(2, byteorder="big")) # Append corrupt bytes
        with open(code_bin, "ab") as f:
            f.write((99).to_bytes(2, byteorder="big")) # Append corrupt bytes
        with open(eval_bin, "ab") as f:
            f.write((99).to_bytes(2, byteorder="big")) # Append corrupt bytes
        ...
        record_log("INFO", f"Write block prevented or failed: {e}")
    ...
    eval_tokens = batcher.read_tokens_from_shard(eval_bin)
